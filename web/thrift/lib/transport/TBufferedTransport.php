...
class TBufferedTransport extends TTransport {
  ...
  public function putBack($data) {
    if (mb_strlen($this->rBuf_,'8bit') === 0) {
      $this->rBuf_ = $data;
    } else {
  ...
  public function readAll($len) {
    $have = mb_strlen($this->rBuf_,'8bit');
    if ($have == 0) {
      ...
      $data = mb_substr($this->rBuf_, 0, $len,'8bit');
      $this->rBuf_ = mb_substr($this->rBuf_, $len, mb_strlen($this->rBuf_, '8bit'), '8bit');
    }
    return $data;
  }
  ...
  public function read($len) {
    ...
    if (mb_strlen($this->rBuf_,'8bit') <= $len) {
      $ret = $this->rBuf_;
      $this->rBuf_ = '';
      return $ret;
    }

    $ret = mb_substr($this->rBuf_, 0, $len,'8bit');
    $this->rBuf_ = mb_substr($this->rBuf_, $len, mb_strlen($this->rBuf_, '8bit'), '8bit');
    return $ret;
  }
  ...
  public function write($buf) {
    $this->wBuf_ .= $buf;
    if (mb_strlen($this->wBuf_,'8bit') >= $this->wBufSize_) {
      $out = $this->wBuf_;
  ...
  public function flush() {
    if (mb_strlen($this->wBuf_,'8bit') > 0) {
